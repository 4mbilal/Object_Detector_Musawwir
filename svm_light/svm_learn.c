     ...
     /* Fills cache for the rows in key */
{
  ...
  for(i=0;i<varnum;i++) {  /* fill up kernel cache */
    cache_kernel_row(kernel_cache,docs,key[i],kernel_parm);		//throws exception
  }
}
        ...
        is removed. */
{
  ...
  kernel_cache->lru[kernel_cache->index[docnum]]=kernel_cache->time; /* lru */
  CFLOAT * ret_mem = ((CFLOAT *)((long long)kernel_cache->buffer + (kernel_cache->activenum*sizeof(CFLOAT)*kernel_cache->index[docnum])));
//  printf("\nOffset = %d", (kernel_cache->activenum*sizeof(CFLOAT)*kernel_cache->index[docnum]));
 // getchar();
//  CFLOAT * ret_mem = ((CFLOAT *)((long)kernel_cache->buffer));
...
{                         /* length of the longest support vector. This is */
  ...
  mWORD nullword;          /* origin of the space. */
