...
long   verbosity = 2;              /* verbosity level (0-4) */
...
SVECTOR *create_svector(mWORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (mWORD *)my_malloc(sizeof(mWORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
	...
	SVECTOR is used */
{
    ...
    sum=(mWORD *)my_malloc(sizeof(mWORD)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (mWORD *)my_malloc(sizeof(mWORD)*(max_words+10));
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (mWORD *)my_malloc(sizeof(mWORD)*(max_words_doc+10));
  if(verbosity>=1) {
...
int parse_document(char *line, mWORD *words, double *label,
